    ...
    while(!QIsEmpty(&q)){
        ...
        for(int i = 0 ; i < 4 ; i++){
            ...
            if(next.y >= 0 && next.y <= column-1 && next.x >= 0 && next.x <= row-1){
                //다음 칸이 벽이고 뚫을 수 있을 때
                if(map[next.y][next.x] == 1 && cur.block > 0 && visited[next.y][next.x][cur.block-1] == 0){
                    next.block = cur.block - 1;
                    Enqueue(&q,next);
                    visited[next.y][next.x][next.block] = visited[cur.y][cur.x][cur.block] + 1;
                }
